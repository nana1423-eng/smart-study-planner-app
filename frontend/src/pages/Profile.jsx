...
export default function Profile() {
  ...
  return (
    <div className="p-8 text-slate-800 dark:text-slate-100 h-full flex flex-col items-center mt-6">
      <div className="max-w-4xl mx-auto w-full space-y-8">
        
        {/* Top Header Card */}
        <div className="bg-white dark:bg-slate-800/40 p-10 rounded-[2.5rem] shadow-sm border border-slate-300 dark:border-slate-700/50 flex flex-col sm:flex-row gap-8 justify-between items-center relative overflow-hidden transition duration-500">
          
          <div className="flex items-center gap-6 relative z-10 w-full sm:w-auto overflow-hidden">
            <div className="w-24 h-24 rounded-full bg-gradient-to-tr from-indigo-500 to-emerald-400 p-1 shadow-sm shrink-0">
              <div className="w-full h-full bg-white dark:bg-slate-800/40 rounded-full flex items-center justify-center text-4xl font-black text-slate-900 dark:text-white uppercase">
                {avatarLetter}
              </div>
            </div>
            <div className="flex-1 min-w-0">
              <h1 className="text-4xl sm:text-5xl font-black text-slate-900 dark:text-white tracking-tight mb-2 truncate">
                {displayName}
              </h1>
              <p className="text-slate-500 dark:text-slate-400 font-bold tracking-widest uppercase text-sm truncate">
                @{user?.username} • {user?.email}
              </p>
            </div>
          </div>
          
          <button onClick={handleLogout} className="bg-rose-50 border border-rose-200 hover:bg-rose-600 hover:text-white text-rose-600 font-black uppercase tracking-widest px-8 py-4 rounded-xl transition-all shadow-sm shrink-0">
            Logout
          </button>
        </div>

        {/* Extended Details Form Module */}
        <div className="bg-white dark:bg-slate-800/40 p-10 rounded-[2.5rem] shadow-sm border border-slate-300 dark:border-slate-700/50 transition duration-500">
           <div className="flex justify-between items-center mb-8 border-b border-slate-200 dark:border-slate-700/50 pb-4">
               <h3 className="text-2xl font-black tracking-tight text-slate-900 dark:text-white">Profile Settings</h3>
               {!isEditing && (
