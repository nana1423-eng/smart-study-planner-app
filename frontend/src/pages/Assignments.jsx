  ...
  return (
    <div className="p-4 sm:p-8 text-slate-800 dark:text-slate-100">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl sm:text-4xl font-black text-slate-900 dark:text-white tracking-tight mb-8">
          Assignments & Tasks
        </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="bg-white dark:bg-slate-800/40 p-6 sm:p-8 rounded-[2rem] border border-slate-300 dark:border-slate-700/50 shadow-sm h-fit relative overflow-hidden transition duration-500 hover:shadow-md hover:border-indigo-100">
            <h2 className="text-xl sm:text-2xl font-black mb-6 text-slate-900 dark:text-white tracking-tight">New Assignment</h2>
          ...
          <div className="lg:col-span-2 space-y-4">
            {assignments.map(a => (
              <div key={a.id} className="bg-white dark:bg-slate-800/40 p-6 rounded-2xl border border-slate-300 dark:border-slate-700/50 transition-all hover:border-slate-300 hover:shadow-md shadow-sm group relative overflow-hidden">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-start ml-2 gap-4">
                  <div className="flex-1">
                    <h3 className={`text-lg sm:text-xl font-bold ${a.status === 'COMPLETED' ? 'text-slate-500 dark:text-slate-400 line-through' : 'text-slate-900 dark:text-white'}`}>{a.title}</h3>
                    <div className="flex gap-2 sm:gap-3 mt-3 text-xs font-bold tracking-wide flex-wrap">
                      <span className="bg-slate-50 dark:bg-[#0f172a] px-2.5 py-1.5 rounded-md text-slate-600 dark:text-slate-300 border border-slate-300 dark:border-slate-700/50">{a.subject?.name}</span>
                      <span className="flex items-center gap-1.5 bg-indigo-900/10 text-indigo-400 px-2.5 py-1.5 rounded-md border border-indigo-100 dark:border-indigo-500/20">
                      ...
                      <span className={`px-2.5 py-1.5 rounded-md border uppercase ${a.priority==='High'?'bg-rose-50 text-rose-600 border-rose-100':a.priority==='Medium'?'bg-amber-50 text-amber-600 border-amber-100':'bg-emerald-50 text-emerald-600 border-emerald-100'}`}>
                        {a.priority}
                      </span>
                    </div>
                  </div>
